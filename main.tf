...
resource "aws_security_group" "k6_instance_sg" {
  name        = "k6-instance-sg"
  description = "Allow SSH inbound traffic for k6 instance"
  vpc_id      = module.vpc.vpc_id
...
resource "aws_instance" "k6_nginx" {
  ...
  vpc_security_group_ids      = [aws_security_group.k6_instance_sg.id]
